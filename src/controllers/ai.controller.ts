...
export const processAICommand = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Construct a detailed system instruction for the AI, emphasizing exact device IDs.
    const systemInstruction = `
      ...
      Format: [{"id": string, "action": "on" | "off", "value"?: any}]
      Constraint: Each action's "id" MUST exactly match an "id" from the Current Home State. Do not invent or modify IDs. Only return the JSON array. No explanations.
    `;
    ...
    // Log how many actions were generated vs. valid
    console.log(`AI generated ${actions.length} actions, ${validActions.length} are valid for the current devices.`);
