...
export const registerUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!username || !email || !password || !householdName) {
      throw new CustomError('Missing required fields: username, email, password, householdName', 400);
    }
    ...
    res.status(201).json({ message: 'User registered successfully and household created.', user: userResponse });
  } catch (error) {
